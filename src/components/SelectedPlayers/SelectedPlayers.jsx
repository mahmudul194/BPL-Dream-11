...
const SelectedPlayers = ({selectedPlayers, setSelectedPlayers, setCoin, coin}) => {
    ...
    return (
        <div className='space-y-5'>
            {
                selectedPlayers.length === 0 ? <div className='h-100 flex items-center justify-center flex-col gap-4'>
                    <h2>No players selected yet</h2>
                    <p>Go to available tab to select players</p>
                </div> :
                selectedPlayers.map((player,index) =>{
                    return (
                        <SelectedPlayerCard player={player} key={index} deleteSelectedPlayer={deleteSelectedPlayer} ></SelectedPlayerCard>
                    )
                })
            }
        </div>
